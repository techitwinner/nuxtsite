    ...
    <div class="page-main">
      <div class="flex-col md:flex-row main-child-row">
        <section class="w-full md:w-[50%] items-start">
          <PageHeading :title="`${$t('pages.blog.tags.oneTag.postsTaggedWith', {tag: tag})}`"/>
        </section>
        <section class="w-full md:w-[50%] gap-2 flex flex-col">
          <nuxt-link v-for="post in posts" :key="post.id" :to="localePath(`/blog/${post.attributes?.slug}`)" class="btn btn-block justify-between">
